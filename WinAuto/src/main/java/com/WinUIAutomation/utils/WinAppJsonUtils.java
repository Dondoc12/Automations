...
public class WinAppJsonUtils {
    ...
    private static String jsonFilePathDefault = WinAppHelpers.getCurrentDir() + "src/test/resources/datajson/store.json";

    private WinAppJsonUtils() {
        super();
    }
    ...
    static {
        try {
            CONFIGMAP = new ObjectMapper().readValue(new File(WinAppHelpers.getCurrentDir() + WinAppConstants.JSON_DATA_FILE_PATH), new TypeReference<HashMap<String, String>>() {
            });
    ...
    public static StringBuffer readJsonFile(String jsonPath) {
        try {
            bufferedReader = new BufferedReader(new FileReader(WinAppHelpers.getCurrentDir() + jsonPath));
