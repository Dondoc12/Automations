...
public class WinAppObjectUtils {
    ...
    public static By getByLocatorFromConfig(String elementName) {
        ...
        String locator = WinAppPropertiesHelpers.getValue(elementName);

        if (locator.isEmpty()) {
            WinAppLogUtils.info("The Locator string " + elementName + " does not exist !!");
            try {
        ...
        WinAppLogUtils.info("Retrieving object of type '" + locatorType + "' and locator '" + locatorValue + "' from the object repository");
    ...
    public static String getXpathValue(String elementName) {
        ...
        if (locator.isEmpty()) {
            try {
                WinAppLogUtils.info("The Locator " + elementName + " does not exist !!");
                throw new Exception("The Locator " + elementName + " does not exist !!");
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            if (!locatorType.toLowerCase().trim().equals("xpath")) {
                try {
                    WinAppLogUtils.info("The Locator Type of " + elementName + " does not XPATH !! => " + locatorType);
                    throw new Exception("The Locator Type of " + elementName + " does not XPATH !! => " + locatorType);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            } else {
                WinAppLogUtils.info("Retrieving Xpath with value '" + locatorValue + "' from the object repository");
                return locatorValue;
            }
        }
    ...
    public static String getXpathDynamic(String xpath, Object... value) {
        if (xpath == null || xpath.equals("")) {
            try {
                WinAppLogUtils.info("Parameter passing error. The 'xpath' parameter is null.");
